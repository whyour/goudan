...
const jdInterceptor = new Interceptor('jd', (context) => {
  context.template.add('jd.success', ['{content}']);
  context.template.add(
    'jd.failed',
    '哎呀，狗蛋获取返利链接失败。等会再来试试？',
  );
})
  .title('京东返利')
  .usage('京东返利')
  .check(async (context, message) => {
    ...
    const jdHealthMobileRegx =
      /https:\/\/mitem.jkcsjd.com\/product\/\d+\.html.*/;
    ...
    if (
      regxs.some((x) => x.test(url)) &&
      (!room || (room && groupIds.includes(room.id)))
    ) {
      return { url };
    }
    return false;
  ...
  .handler(async (context, message, checkerArgs) => {
    ...
    let msg = '';
    const [promotion, result] = await Promise.all([
      getPromotion(encodeUrl),
      getHistoryPrice(encodeUrl),
    ]);
    if (result || promotion) {
      let titleStr = '';
      if (result.title) {

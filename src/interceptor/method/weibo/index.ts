/**
 * 热搜查询
 * 提供新浪微博实时热搜查询，默认提供的是挖数据（wapi）的实现。
 */
...
const weiboInterceptor = new Interceptor('weibo', (context) => {
  ...
  .check((context, message) => {
    if (
      /^狗蛋/.test(message.text()) &&
      /[查看有]?(微博)?热搜/.test(message.text())
    )
      return true;
  })
  .handler(async (context) => {
    ...
    return context.template.use('weibo.success', {
      hotSearch: hotSearchList
        .map(
          (h) =>
            `${h.key}${h.isNew ? Emoji.get('new') : ''}${
              h.isHot ? Emoji.get('fire') : ''
            }${h.isExplosive ? Emoji.get('boom') : ''}${
              h.isBoiled ? Emoji.get('hotsprings') : ''
            }`,
        )
        .join('\n'),
    });

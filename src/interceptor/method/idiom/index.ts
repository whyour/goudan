...
function shuffleFindIdiomByFirstPinyin(
  pinyin: string,
  excepted?: string[],
): convertedIdiom | void {
  const results = [];
  for (const i of idiomList) {
    if (i.pinyin[0][0] === pinyin && (!excepted || !excepted.includes(i.name)))
      results.push(i);
  }
  if (results.length > 0)
...
const idiomInterceptor = new Interceptor('idiom', (context) => {
  ...
  if (
    !fs.existsSync(
      path.join(context.__data_dir, './idioms/idioms_converted.json'),
    )
  ) {
    if (!fs.existsSync(path.join(context.__data_dir, './idioms/idioms.json'))) {
      console.error(
        '您既没有下载源成语库数据文件，也没有下载转换后的成语库数据文件，所以成语接龙功能将处于不可用状态。',
      );
      console.log(
        '您可以运行tools目录下的idiom.ts来下载源成语库，也可以在本项目仓库的Github Release中下载资源文件。',
      );
    } else {
      const oriIdiomsData = fs.readFileSync(
        path.join(context.__data_dir, './idioms/idioms.json'),
        'utf-8',
      );
  ...
  .check((context, message) => {
    ...
    if (/^狗蛋/.test(message.text()) && /成语接龙/.test(message.text())) {
      return {
        ...args,
        action: 'start',
      };
    ...
    if (action === 'start') {
      ...
      if (!gameSpace[spaceId] || gameSpace[spaceId].status === 'idle') {
        ...
        gameSpace[spaceId].on('start', async () => {
          ...
          await message.say(
            context.template.use('idiom.game.start', {
        ...
        gameSpace[spaceId].on('answer', async (answer) => {
          ...
          const maybeAnswer = (function () {
            const contentPinyin = pinyin(answer.content, {
              style: pinyin.STYLE_NORMAL,
            });
          ...
          if (idiom && isEndToEnd(lastIdiom, idiom)) {
            ...
            let nextIdiom,
              count = 0;
            do {
              nextIdiom = shuffleFindIdiomByFirstPinyin(
                idiom.pinyin[idiom.pinyin.length - 1][count],
              );
              count++;
            } while (
              !nextIdiom &&
              count < idiom.pinyin[idiom.pinyin.length - 1].length
            );
            if (nextIdiom) {
              gameSpace[spaceId].push(nextIdiom);
              await message.say(
                context.template.use('idiom.game.next', {
            ...
            await message.say(
              context.template.use('idiom.game.wrong', {
        ...
        gameSpace[spaceId].on('end', async (winner: string = 'bot') => {
          ...
          const medalEmoji = [
            Emoji.get('first_place_medal'),
            Emoji.get('second_place_medal'),
            Emoji.get('third_place_medal'),
          ];
          const scoreResult = scoreArray.length
            ? scoreArray
                .map(
                  (v, index) =>
                    `${index <= 2 ? medalEmoji[index] : ''}${v.name}：${
                      v.score
                    }分`,
                )
                .join('\n')
          ...
          if (winner === 'bot') {
            await message.say(
              context.template.use('idiom.game.botWin', {
                score: scoreResult,
              }),
            );
          } else if (winner === 'player') {
            await message.say(
              context.template.use('idiom.game.playerWin', {
                score: scoreResult,
              }),
            );
          }

...
async function getIdioms(): Promise<string[]> {
  ...
  for (const c of capital) {
    ...
    do {
      ...
      const idiomMatch = data.match(
        /<li><a\s*href=".*?"\s*title=".*?">(.*?)<\/a><\/li>/g,
      );
      for (const idiomMatchRow of idiomMatch) {
        const idiom = idiomMatchRow.match(
          /<li><a\s*href=".*?"\s*title=".*?">(.*?)<\/a><\/li>/,
        )[1];
        idioms.push(idiom);
      }
      const nextPageMatch = data.match(
        /<a\s*class="next"\s*href="(.*?)">下一页<\/a>/,
      );
      const next = nextPageMatch
        ? `http://tools.2345.com/chengyu/${c}/${nextPageMatch[1]}`
        : null;
...
getIdioms().then((idioms) => {
  ...
  fs.writeFileSync(
    path.join(__dirname, '../data/idioms/idioms.json'),

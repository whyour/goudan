...
export class MessageProcessor {
  ...
  public async interceptor(interceptor: Interceptor): Promise<void> {
    ...
    this.interceptors
      .map((i) => i.$alias)
      .forEach((a) => (a ? usedAlias.push(...a) : void 0));
    const verifyExists = (str: string) => {
      if (usedName.includes(str))
        throw new Error(
          `Cannot insert interceptor: ${str} already exists as an interceptor name`,
        );
      if (usedTitle.includes(str))
        throw new Error(
          `Cannot insert interceptor: ${str} already exists as an interceptor title`,
        );
      if (usedAlias.includes(str))
        throw new Error(
          `Cannot insert interceptor: ${str} already exists as an interceptor alias`,
        );
    };
    verifyExists(interceptor.$name);
    verifyExists(interceptor.$title);
    if (interceptor.$alias && interceptor.$alias.length)
  ...
  public async process(
    ...
    const send = (content: MessageSayType | void) =>
      new Promise<void | Message>((resolve, reject) => {
        if (
          content === undefined ||
          content === null ||
  ...
  public async usages(message?: Message): Promise<string[]> {
    ...
    for (const i of this.interceptors) {
      if (i.$usage)
        results.push(
          `${i.$title}：${
            typeof i.$usage === 'string'
              ? i.$usage
              : await i.$usage(this.context, message)
          }`,
        );
    }
    return results;
  ...
  public async usage(name: string, message?: Message): Promise<string | void> {
    for (const i of this.interceptors) {
      if (i.$name === name || i.$title === name || i.$alias.includes(name))
        return `${i.$name}\n${
          typeof i.$usage === 'string'
            ? i.$usage
            : await i.$usage(this.context, message)
  ...
  public async attribute(
    ...
    {
      desc?: string;
      data: string | number | boolean | any[] | Record<any, any>;
    }
  > | void> {
    for (const i of this.interceptors) {
      if (i.$name === name || i.$title === name || i.$alias.includes(name))
        return await i.$attributes(this.context, ...args);
    }
    return void 0;
  }
  ...
  > {
    ...
    for (const i in this.interceptors) {
      result[this.interceptors[i].$name] = await this.interceptors[
        i
      ].$attributes(this.context, ...args);
    }
    return result;
  }
}

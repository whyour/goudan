...
import caiyunWeather, {
  ...
  toWindDirectionDesc,
  toWindSpeedDesc,
} from '../interceptor/method/weather/caiyunapi';
...
async function weather(pos) {
  ...
  const data = await caiyunWeather(
  ...
  }℃\n空气质量：${data.result.realtime.air_quality.aqi.chn}（${
    data.result.realtime.air_quality.description.chn
  }）\n舒适指数：${data.result.realtime.life_index.comfort.index}（${
    data.result.realtime.life_index.comfort.desc
  }）\n紫外线指数：${data.result.realtime.life_index.ultraviolet.index}（${
    data.result.realtime.life_index.ultraviolet.desc
  }）\n风力：${toWindSpeedDesc(
    data.result.realtime.wind.speed,
  )}\n风向：${toWindDirectionDesc(

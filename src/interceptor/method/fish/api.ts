...
export function genText() {
  ...
  let result = [
    `【摸鱼办】提醒您：${now_month} 月 ${now_day} 日，${get_midday(
      now.hour(),
    )}好，摸鱼人！`,
    '工作再累，一定不要忘记摸鱼哦！',
    choice([
    ...
    '',
    // Weekend
    (0 <= weekday && weekday < 6 && `距离周末还有${6 - weekday}天`) ||
      `好好享受周末吧\n`,
  ];
  ...
  for (let item of festivals) {
    ...
    if (fest_month == now_month && fest_day == now_day) {
      ...
      const fest_date = dayjs()
        .month((fest_month as number) - 1)
        .date(fest_day as number);

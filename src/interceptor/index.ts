  ...
  for (const n of enabledInterceptorList) {
    const interceptor = await (async function () {
      try {
        ...
        else if (fs.existsSync(path.join(__data_dir, `./interceptor/${n}`)))
          return (await import(path.join(__data_dir, `./interceptor/${n}`)))
            .default;
        else if (fs.existsSync(path.join(__data_dir, `./interceptor/${n}.js`)))
          return (await import(path.join(__data_dir, `./interceptor/${n}.js`)))
            .default;

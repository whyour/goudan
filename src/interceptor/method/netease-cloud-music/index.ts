...
function getSearchArgs(
  text: string,
): null | { type: SearchType; keywords: string } {
...
const neteaseMusicInterceptor = new Interceptor(
  ...
  (context) => {
    context.template.add(
      'netease-cloud-music.search.failed',
      '很抱歉，搜索 {search} 时失败了，请你等会再试试吧。',
    );
    context.template.add(
      'netease-cloud-music.search.success',
      '狗蛋给你找到了一首歌~\n{title} - {artists}\n{url}',
    );
  },
)
  ...
  .check((context, message) => {
    ...
    if (/^狗蛋/.test(message.text()) && searchArgs) {
      return { searchArgs };
    }
  })
  .handler(
    async (
      context,
      message,
      checkerArgs: { searchArgs: { type: SearchType; keywords: string } },
    ) => {
      const { searchArgs } = checkerArgs;
      // 调用搜索API
      const searchResult: NeteaseCloudMusicTypes.SearchResponse =
        (await NeteaseCloudMusic.search({
          ...searchArgs,
        })) as NeteaseCloudMusicTypes.SearchResponse;
      if (
        searchResult.status === 200 &&
        searchResult.body.result.songs.length > 0
      ) {
        const resultSong = searchResult.body.result.songs[0];
        // 调用歌曲详情API
        const resultSongDetailResponse: NeteaseCloudMusicTypes.SongDetailResponse =
          (await NeteaseCloudMusic.song_detail({
            ids: `${resultSong.id}`,
          })) as NeteaseCloudMusicTypes.SongDetailResponse;
        if (
          resultSongDetailResponse.status === 200 &&
          resultSongDetailResponse.body.songs.length > 0
        ) {
          ...
          // console.warn(e)
          await message.say(
            context.template.use('netease-cloud-music.search.success', {
              title,
              artists,
              url,
            }),
